...
class Appstore extends Component {
  ...
  render() {
    ...
    const filteredList = appsList.filter(eachApp =>
      eachApp.appName
        .toLocaleLowerCase()
        .includes(searchInput.toLocaleLowerCase()),
    )
    const filteredListOnTabs = appsList.filter(
      eachApp => eachApp.category === activeTabId,
    )
    const finalList = searchInput.length > 0 ? filteredList : filteredListOnTabs
